...
- [ ] 编写 GitHub Actions 脚本：每周抓取学校校车 PDF，转换为 `bus.json` 并发布到 GitHub Pages
- [ ] CLI 端拉取 JSON 并用 Rich 渲染漂亮的时刻表
- [ ] 实现 `campus bus` 命令（支持 `--from`、`--to`、`--time` 等过滤参数）

```python
# 预期命令示例
$ campus bus --from 九龙湖 --to 四牌楼
┌──────────────────────────────────────────────┐
│ 🚌 校车时刻表：九龙湖 → 四牌楼               │
├──────────┬────────────┬──────────────────────┤
│ 发车时间  │ 发车站点    │ 备注                 │
