...
TOOLS = [
    {
        ...
        "function": {
            "name": "venue_list",
            "description": (
                "列出指定类型的所有可预约场馆。返回场馆名称、编号、校区、容量等信息。"
            ),
            "parameters": {
                "type": "object",
                "properties": {
            ...
            "parameters": {
                ...
                "properties": {
                    "venue_id": {
                        "type": "string",
                        "description": (
                            "场馆 UUID。如果只有编号（如 JLH01），"
                            "先调用 venue_list 获取 UUID。"
                        ),
                    },
                    "date": {
                        "type": "string",
    ...
    },
    {
        ...
        "function": {
            ...
            "description": (
                "批量查询多个场馆在指定日期的时段可用情况。"
                "适合一次性查看某个校区所有场地的空闲状态。"
            ),
            "parameters": {
                "type": "object",
                "properties": {
    ...
    },
    {
        ...
        "function": {
            "name": "venue_book",
            "description": (
                "提交场馆预约。需要指定场馆 UUID、日期、时段。可通过编号查找场馆。"
            ),
            "parameters": {
                "type": "object",
                "properties": {
...
async def execute_tool(adapter: VenueAdapter, func_name: str, args: dict) -> str:
    """执行工具调用并返回 JSON 字符串。"""
    try:
        if func_name == "venue_list":
            ...
            return json.dumps([v.model_dump() for v in venues], ensure_ascii=False)
            ...
            if len(venue_id) < 36:
                venues = await adapter.get_venues(type_name)
                matched = [v for v in venues if v.number.upper() == venue_id.upper()]
                if not matched:
                    return json.dumps(
            ...
            return json.dumps([b.model_dump() for b in bookings], ensure_ascii=False)
...
- 羽毛球场编号规则：JLH01-JLH13(九龙湖)、SPL01-SPL04(四牌楼)、\
DJQ001/DJQ003(丁家桥)、WX02-WX10(无锡)
        ...
        while True:
            ...
            if msg.tool_calls:
                ...
                for tc in msg.tool_calls:
                    ...
                    print(
                        f"  🔧 {func.name}({json.dumps(args, ensure_ascii=False)[:80]})"
                    )
